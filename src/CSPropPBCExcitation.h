...
/*!
  ...
  */
class CSXCAD_EXPORT CSPropPBCExcitation : public CSProperties
{

public:
    CSPropPBCExcitation(ParameterSet* paraSet,unsigned int number=0);
    CSPropPBCExcitation(CSProperties* prop);
    CSPropPBCExcitation(unsigned int ID, ParameterSet* paraSet);
    virtual ~CSPropPBCExcitation();

    //! Get PropertyType as a xml element name \sa PropertyType and GetType
    virtual const std::string GetTypeXMLString() const {return std::string("PBCExcitation");}
    ...
    virtual void Init();
    virtual bool Update(std::string *ErrStr=NULL, bool type=0);
...
protected:
    ...
    ParameterScalar SINExcitation[3];		// excitation amplitude vector
